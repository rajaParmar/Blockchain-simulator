...
adversary_hash = float(sys.argv[5])
def start_client(exp_parameter,peer_id):
    ...
    # print(adversary_count)
    print(len(H))
    frac = adversary_count/len(H)
    print("fraction")
    print(frac)
    final_string='adv'+str(int(itime)) + '_' + str(adversary_hash)+'_'+str(total_blocks_required)
...
total_honest_clients = int(sys.argv[6])
# 1 is adversary 4 are honest
itime = float(sys.argv[3])
def start_all_clients(total_honest_clients):
    hashpower = (1.0-adversary_hash)/total_honest_clients
...
# total_clients = 2
start_all_clients(total_honest_clients)
